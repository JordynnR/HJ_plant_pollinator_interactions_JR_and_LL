...
```{r}
ggsave("final_visuals/native_bees_AB_test_hist.png", plot=test_hist)
```
...
for (i in 1:1000){
  ...
  test_w_apis<- append(test_w_apis, one_test_w_apis)
}

#p-value with non-native bees included
p_val_w_apis <- mean(abs(test_w_apis) >= abs(obs_w_apis))
p_val_w_apis
#gives 0.879= even closer to null

# convert to dataframe to plot
test_w_apis_df<- as.data.frame(test_w_apis)

# plot new histogram when including non-native bees
ggplot(test_w_apis_df) +
  geom_histogram(aes(x=test_w_apis)) +
  geom_point(aes(x=obs_w_apis, y=0),color="red")
